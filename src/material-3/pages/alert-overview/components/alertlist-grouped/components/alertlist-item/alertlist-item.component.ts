...
export const AlertlistSubItem = (vm: AlertlistSubItemVM) =>
  mdListItem(
    ...
    div(
      ...
      mdIcon({ class: priorityToTextColorClass(vm.priority) }, vm.priority),
      vm.label
    ),
    div(
...
export const AlertlistItem = (vm: AlertlistItemVM) =>
  main(
    ...
    mdListItem(
      ...
      mdIconButton({ slot: "end" }, mdIcon("more_vert")),
      mdIconButton(
        { slot: "end" },
        mdIcon(vm.open ? "keyboard_arrow_down" : "keyboard_arrow_up")
      )
    ),
