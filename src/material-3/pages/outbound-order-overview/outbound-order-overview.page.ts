...
export const OutboundOrderOverviewPage = (vm: OutboundOrderOverviewPageVM) =>
  main(
    ...
    section({
      class: 'flex flex-row gap-2', style: 'justify-content: space-evenly'
    },
      div({
        ...
        thead(
          tr(
            th({ class: "px-3", style: 'width: 6rem;' },
              mdCheckbox()),
            vm.datatable.titles.map(header =>
              th(header)
            )
          )
        ),
    ...
    ul({ class: 'flex', style: 'margin-left:auto' },
      li(button('Rows per page: 10', mdIcon('close'),
      )),
